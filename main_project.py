...
if compute_fisher == True:
    ...
    fisher_dict = lib.get_fisher_dict(
        spectra_dict, angle_fisher_array, w_inv, beam_fisher_array, foregrounds=foregrounds)
...
if plot_truncated_fisher_cumlulative == True and compute_fisher == True and compute_fisher_trunc == True:
    if foregrounds == True:
        plotpro.truncated_fisher_cumulative(
            fisher_trunc_array_dict, ((3*u.deg, (w_inv, 30. * u.arcmin)), 'foregrounds'))
    else:
        plotpro.truncated_fisher_cumulative(
            fisher_trunc_array_dict, (imposed_rotation_angle*u.deg, 'no noise'))
...
if plot_cumulative_3D == True and compute_fisher == True:
    if foregrounds == True:
        surface, heatmap = plotpro.cumulative_error_3D(fisher_dict[(
            (3*u.deg, (w_inv, 30. * u.arcmin)), 'foregrounds')], ((3*u.deg, (w_inv, 30. * u.arcmin)), 'foregrounds'))
    else:
        surface, heatmap = plotpro.cumulative_error_3D(
            fisher_dict[(imposed_rotation_angle*u.deg, 'no noise')], (imposed_rotation_angle*u.deg, 'no noise'))
...
if comp_foregrounds == True and foregrounds == True and compute_fisher == True:
    ...
    fig_cumu = plotpro.cumulative_error(fisher_dict[(
        3*u.deg, (w_inv, 30. * u.arcmin))], label='no foregrounds, lmin={}'.format(l_min_range[0]), l_min=l_min_range[0], color=color)

    fig_cumu = plotpro.cumulative_error(fisher_dict[((3*u.deg, (w_inv, 30. * u.arcmin)), 'foregrounds')],
                                        label='foregrounds, lmin={}'.format(l_min_range[0]), l_min=l_min_range[0], dotted=True, color=color)
    for l in l_min_range[1:]:
        color = next(plt.gca()._get_lines.prop_cycler)['color']

        fig_cumu = plotpro.cumulative_error(fisher_dict[(
            3*u.deg, (w_inv, 30. * u.arcmin))], label='no foregrounds, lmin={}'.format(l), l_min=l, color=color)
        fig_cumu = plotpro.cumulative_error(fisher_dict[(
            (3*u.deg, (w_inv, 30. * u.arcmin)), 'foregrounds')], label='foregrounds, lmin={}'.format(l), l_min=l, color=color, dotted=True)
    plt.title('cumulative error for rotation {}, noise beam {} and with and without foregrounds and different lmin'.format(
        3*u.deg, 30. * u.arcmin))
    plt.show()
...
if test_likelihood == True:
    ...
    if extra_noise:
        ...
        test_spectra_cov = copy.deepcopy(
            spectra_dict[((data_angle, (w_inv, 30. * u.arcmin)), 'foregrounds')]) + test_noise

    plot_increment_spectra = 0
    if plot_increment_spectra:
        plotpro.spectra({'CMB w rotation and lensing': spectra_dict[data_angle], 'noise': spectra_dict[(
            w_inv, 30. * u.arcmin)], 'foregrounds': spectra_dict['foregrounds'], 'extra noise': test_noise, 'all': test_spectra_cov}, linear_cross=False)
        plt.show()
    ...
    for angle in angle_array[1:]:
        likelihood_angle.append(lib.likelihood(
            spectra_dict[(angle, 'unlensed')], spectra_dict[(data_angle, 'unlensed')]))
        likelihood_angle_lensing.append(lib.likelihood(
            spectra_dict[angle], spectra_dict[data_angle]))
        noise_counter = 0
        for w_inv in w_inv_array:
            likelihood_angle_lensing_noise_list[noise_counter].append(lib.likelihood(
                spectra_dict[(angle, (w_inv, 30 * u.arcmin))], spectra_dict[(data_angle, (w_inv, 30 * u.arcmin))]))
            noise_counter += 1
        likelihood_angle_lensing_noise_foregrounds.append(lib.likelihood(spectra_dict[(
            angle, (w_inv, 30. * u.arcmin))], spectra_dict[((data_angle, (w_inv, 30. * u.arcmin)), 'foregrounds')]))
        likelihood_angle_lensing_noise_foregrounds10.append(lib.likelihood(spectra_dict[(
            angle, (w_inv, 30. * u.arcmin))], spectra_dict[((data_angle, (w_inv, 30. * u.arcmin)), 'foregrounds/10')]))
        likelihood_angle_lensing_noise_foregrounds100.append(lib.likelihood(spectra_dict[(
            angle, (w_inv, 30. * u.arcmin))], spectra_dict[((data_angle, (w_inv, 30. * u.arcmin)), 'foregrounds/100')]))
        likelihood_angle_lensing_noise_foregrounds1000.append(lib.likelihood(spectra_dict[(
            angle, (w_inv, 30. * u.arcmin))], spectra_dict[((data_angle, (w_inv, 30. * u.arcmin)), 'foregrounds/1000')]))

        if extra_noise:
            test_spectra = copy.deepcopy(
                spectra_dict[((angle, (w_inv, 30. * u.arcmin)), 'foregrounds')]) + test_noise

        if extra_noise:
            likelihood_angle_lensing_noise_foregrounds_100uKnoise.append(lib.likelihood(
                spectra_dict[((angle, (w_inv, 30. * u.arcmin)), 'foregrounds')], test_spectra_cov))
    ...
    test_lnL = 0
    if test_lnL == 0:
        # (likelihood_no_noise -min(likelihood_no_noise))/ (max(likelihood_no_noise)-min(likelihood_no_noise))
        likelihood_angle = np.exp(-np.array(lib.likelihood_normalisation(likelihood_angle)))
        # (likelihood_no_foregrounds -min(likelihood_no_foregrounds))/ (max(likelihood_no_foregrounds)-min(likelihood_no_foregrounds))
        likelihood_angle_lensing = np.exp(
            -np.array(lib.likelihood_normalisation(likelihood_angle_lensing)))
        noise_counter = 0
        for i in range(len(w_inv_array)):
            # (likelihood_noise -min(likelihood_noise))/ (max(likelihood_noise)-min(likelihood_noise))
            likelihood_angle_lensing_noise_list[i] = np.exp(
                -np.array(lib.likelihood_normalisation(likelihood_angle_lensing_noise_list[i])))

        likelihood_angle_lensing_noise_foregrounds = np.exp(
            -np.array(lib.likelihood_normalisation(likelihood_angle_lensing_noise_foregrounds)))
        likelihood_angle_lensing_noise_foregrounds10 = np.exp(
            -np.array(lib.likelihood_normalisation(likelihood_angle_lensing_noise_foregrounds10)))
        likelihood_angle_lensing_noise_foregrounds100 = np.exp(
            -np.array(lib.likelihood_normalisation(likelihood_angle_lensing_noise_foregrounds100)))
        likelihood_angle_lensing_noise_foregrounds1000 = np.exp(
            -np.array(lib.likelihood_normalisation(likelihood_angle_lensing_noise_foregrounds1000)))

        if extra_noise:
            # (likelihood_test -min(likelihood_test))/ (max(likelihood_test)-min(likelihood_test))
            likelihood_angle_lensing_noise_foregrounds_100uKnoise = np.exp(
                -np.array(lib.likelihood_normalisation(likelihood_angle_lensing_noise_foregrounds_100uKnoise)))
    ...
    for i in range(len(w_inv_array)):
        plt.plot(angle_array[1:], likelihood_angle_lensing_noise_list[i],
                 label='+= intrumental noise, w_inv={}'.format(w_inv_array[i]), alpha=alpha_plot)
    # plt.plot(angle_array, likelihood_angle_lensing_noise_foregrounds,':',label='+= foregrounds')
    if extra_noise:
        plt.plot(angle_array, likelihood_angle_lensing_noise_foregrounds_100uKnoise,
    ...
    if test_gaussian:
        fisher = lib.fisher_angle(
            spectra_dict[0*u.deg], data_angle, cl_rot=spectra_dict[data_angle],  return_elements=False) / (u.rad * u.rad)
        print('fisher rad =', fisher)
        fisher_unlensed_unlensed = lib.fisher_angle(spectra_dict[(
            0*u.deg, 'unlensed')], data_angle, cl_rot=spectra_dict[(data_angle, 'unlensed')],  return_elements=False) / (u.rad * u.rad)
        fisher_unlensed_lensed = lib.fisher_angle(spectra_dict[(
            0*u.deg, 'unlensed')], data_angle, cl_rot=spectra_dict[data_angle],  return_elements=False) / (u.rad * u.rad)
        fisher_lensed_lensed = lib.fisher_angle(
            spectra_dict[0*u.deg], data_angle, cl_rot=spectra_dict[data_angle],  return_elements=False) / (u.rad * u.rad)
        fisher_lensed_unlensed = lib.fisher_angle(spectra_dict[0*u.deg], data_angle, cl_rot=spectra_dict[(
            data_angle, 'unlensed')],  return_elements=False) / (u.rad * u.rad)
    ...
    if likelihood_presentation == 1:
        ...
        plt.plot(angle_array[1:], likelihood_angle_lensing_noise_list[-1],
                 label='Lensed + intrumental noise, w_inv={}'.format(w_inv_array[-1]), alpha=alpha_plot)
        plt.axvline(x=data_angle.value, linestyle=':', label='Input rotation angle', color='k')
        plt.legend(title='Included in model:', loc='upper left',
                   fontsize='small', bbox_to_anchor=(1, 0.5))
        plt.title('Likelihood Estimation for different models and data')
        ...
        plt.plot(angle_array[1:], likelihood_angle_lensing_noise_list[-1],
                 label='Lensed + intrumental noise, w_inv={}'.format(w_inv_array[-1]), alpha=alpha_plot)
        plt.plot(angle_array[1:], likelihood_angle_lensing_noise_foregrounds,
                 label='Lensed + noise + foregrounds in data not model', alpha=alpha_plot)
        plt.axvline(x=data_angle.value, linestyle=':', label='Input rotation angle', color='k')
        plt.legend(title='Included in model:', loc='upper left',
                   fontsize='small', bbox_to_anchor=(1, 0.5))
        plt.title('Likelihood Estimation for different models and data')
        ...
        plt.plot(angle_array[1:], likelihood_angle_lensing_noise_foregrounds10,
                 label=r'Lensed + noise + $\frac{foregrounds}{10}$ in data not model', alpha=alpha_plot)
        plt.axvline(x=data_angle.value, linestyle=':', label='Input rotation angle', color='k')
        plt.legend(title='Included in model:', loc='upper left',
                   fontsize='small', bbox_to_anchor=(1, 0.5))
        plt.title('Likelihood Estimation for different models and data')
        ...
        plt.plot(angle_array[1:], likelihood_angle_lensing_noise_foregrounds100,
                 label=r'Lensed + noise + $\frac{foregrounds}{100}$ in data not model', alpha=alpha_plot)
        plt.axvline(x=data_angle.value, linestyle=':', label='Input rotation angle', color='k')
        plt.legend(title='Included in model:', loc='upper left',
                   fontsize='small', bbox_to_anchor=(1, 0.5))
        plt.title('Likelihood Estimation for different models and data')
        ...
        plt.plot(angle_array[1:], likelihood_angle_lensing_noise_foregrounds1000,
                 label=r'Lensed + noise + $\frac{foregrounds}{1000}$ in data not model', alpha=alpha_plot)
...
if spectra_presentation:
    ...
    plt.show()
    plotpro.spectra({'normal power spectra': spectra_dict[(0*u.deg, 'unlensed')], 'power spectra rotated by {}'.format(
        imposed_rotation_angle*u.deg): spectra_dict[(imposed_rotation_angle*u.deg, 'unlensed')]})
    plt.show()
