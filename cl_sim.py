...
def main():
    ...
    if map_creation:
        ...
        Cl_noise = cl_lib.get_nsim_freq_cl(mask*noise_maps, nsim, frequencies_index, mask,
    ...
    if Cl_estimation:
        start_Cl = time.time()
        Cl_cmb = cl_lib.get_nsim_freq_cl(mask*cmb_map_nsim, nsim, frequencies_index, mask,
                                         mask_apo, purify_e, purify_b, wsp)
        cmb_dust_map = np.add(cmb_map_nsim, dust_map_freq)
        Cl_cmb_dust = cl_lib.get_nsim_freq_cl(mask*cmb_dust_map, nsim,
                                              frequencies_index, mask, mask_apo,
                                              purify_e, purify_b, wsp)

        cmb_sync_map = np.add(cmb_map_nsim, sync_map_freq)
        Cl_cmb_sync = cl_lib.get_nsim_freq_cl(mask*cmb_sync_map, nsim,
                                              frequencies_index, mask, mask_apo,
                                              purify_e, purify_b, wsp)

        cmb_dust_sync_map = np.add(cmb_dust_map, sync_map_freq)
        Cl_cmb_dust_sync = cl_lib.get_nsim_freq_cl(mask*cmb_dust_sync_map, nsim,
                                                   frequencies_index, mask, mask_apo,
                                                   purify_e, purify_b, wsp)
    ...
    if minimisation:
        ...
        fit_alpha_cmb, H_cmb = cl_lib.min_and_error_nsim_freq(
            spectra_cl, Cl_cmb, nsim, frequencies_index, b, nside, f_sky_SAT,
            spectra_used='all', spectra_indexation='NaMaster',
            minimisation_init=0.001,
            compute_error68=False, step_size=1e-5, output_grid=False)
        fit_alpha_cmb_dust, H_cmb_dust = cl_lib.min_and_error_nsim_freq(
            spectra_cl, Cl_cmb_dust, nsim, frequencies_index, b, nside, f_sky_SAT,
            spectra_used='all', spectra_indexation='NaMaster',
            minimisation_init=0.001,
            compute_error68=False, step_size=1e-5, output_grid=False)
        fit_alpha_cmb_sync, H_cmb_sync = cl_lib.min_and_error_nsim_freq(
            spectra_cl, Cl_cmb_sync, nsim, frequencies_index, b, nside, f_sky_SAT,
            spectra_used='all', spectra_indexation='NaMaster',
            minimisation_init=0.001,
            compute_error68=False, step_size=1e-5, output_grid=False)
        fit_alpha_cmb_dust_sync, H_cmb_dust_sync = cl_lib.min_and_error_nsim_freq(
            spectra_cl, Cl_cmb_sync, nsim, frequencies_index, b, nside, f_sky_SAT,
            spectra_used='all', spectra_indexation='NaMaster',
            minimisation_init=0.001,
            compute_error68=False, step_size=1e-5, output_grid=False)
